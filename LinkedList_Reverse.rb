...
class LinkedList
  ...
  def reverse! (head = @head, previous = nil)
    if head == nil
      @head = previous
      return self
    end
  ...
  def dup
    ...
    node = @head
    while node != nil
      result.append(Node.new(node.value))
      node = node.next
    end
    return result
  end
...
p list.reverse!.to_s #=> (25, 10, 15)
p list.reverse.to_s #=> (15, 10, 25)
p list.to_s #=> (25, 10, 15)
